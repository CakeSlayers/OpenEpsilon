package studio.coni.epsilon.language

import studio.coni.epsilon.EpsilonPlus
import studio.coni.epsilon.module.setting.TextSetting
import java.io.File
...
object TextManager {
    ...
    private inline fun mkdirs() {
        File(EpsilonPlus.DEFAULT_CONFIG_PATH + "language").mkdirs()
    }

    private inline fun saveLanguage(standardName: String) {
        val file = File(EpsilonPlus.DEFAULT_CONFIG_PATH + "language/$standardName.txt")
        if (!file.exists()) {
            mkdirs()
            file.createNewFile()
        }
    ...
    private inline fun readLanguage(standardName: String) {
        File(EpsilonPlus.DEFAULT_CONFIG_PATH + "language/$standardName.txt").readLines().forEach { s ->

...
import studio.coni.epsilon.module.setting.FontSetting
...
object FlatTheme : IDefaultBothTheme, SpartanScreen() {
    ...
    override fun panel(panel: Panel, mouseX: Int, mouseY: Int, partialTicks: Float) {
        with(panel) {
            ...
            //Title
            drawString(category.showName, translatedX + (width / 2f - studio.coni.epsilon.management.Fonts.boldFont.getWidth(category.showName, FontSetting.size) / 2f), translatedY + height / 2f - studio.coni.epsilon.management.Fonts.boldFont.getHeight(
                FontSetting.size) / 2f, white, FontSetting.size, font = studio.coni.epsilon.management.Fonts.boldFont)

package studio.coni.epsilon.menu.main

import studio.coni.epsilon.EpsilonPlus
import studio.coni.epsilon.gui.SpartanScreen
import studio.coni.epsilon.management.Fonts
import studio.coni.epsilon.menu.alt.AltManagerGui
import studio.coni.epsilon.menu.main.elements.MainSelectButton
...
object MainMenu : SpartanScreen() {
    ...
    private fun MainSelectButton.getSelected(mouseX: Int, mouseY: Int): Boolean {
        val isController = currentControllerType != ControllerType.Mouse && currentElement == this
        val isMouse = currentControllerType == ControllerType.Mouse
        val isHoovered = this.isHoovered(mouseX, mouseY)
        if (isHoovered) {
            categoryPosition = if (buttons.indexOf(this) != -1) 0
            else 1
            currentElement = this
            if (!isMouse) currentControllerType = ControllerType.Mouse
        }

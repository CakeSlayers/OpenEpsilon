...
import studio.coni.epsilon.util.graphics.buffer.DynamicVAO
import studio.coni.epsilon.util.graphics.mask.EnumFacingMask
import studio.coni.epsilon.util.graphics.shaders.Shader
...
object RenderUtils3D : Helper {
    ...
    private var depth = glIsEnabled(GL_LIGHTING)
    private var texture = glIsEnabled(GL_BLEND)
    private var clean = glIsEnabled(GL_TEXTURE_2D)
    private var bind = glIsEnabled(GL_DEPTH_TEST)
    private var override = glIsEnabled(GL_LINE_SMOOTH)
    ...
    fun glPre(lineWidth: Float) {
        depth = glIsEnabled(GL_LIGHTING)
        texture = glIsEnabled(GL_BLEND)
        clean = glIsEnabled(GL_TEXTURE_2D)
        bind = glIsEnabled(GL_DEPTH_TEST)
        override = glIsEnabled(GL_LINE_SMOOTH)
    ...
    fun jelloRender(entity: EntityLivingBase) {
        ...
        glTranslated(-mc.renderManager.viewerPosX, -mc.renderManager.viewerPosY, -mc.renderManager.viewerPosZ)
        ...
        for (i in 0..360) {
            ...
            glVertex3d(posX2, posY + yPos + moveFace + 1e-9, posZ2)
            glColor4f(1f, 1f, 1f, 0.4F)
            glVertex3d(posX2, posY + yPos, posZ2)
        }
        glEnd()
    ...
    fun drawFilledBox(axisAlignedBB: AxisAlignedBB) {
        bufferbuilder.begin(GL_QUADS, DefaultVertexFormats.POSITION_NORMAL)
        run {

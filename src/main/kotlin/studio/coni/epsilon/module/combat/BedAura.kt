...
import studio.coni.epsilon.common.collections.CircularArray
...
import studio.coni.epsilon.concurrent.BackgroundScope
...
import studio.coni.epsilon.util.inventory.executedOrTrue
import studio.coni.epsilon.util.inventory.inventoryTask
import studio.coni.epsilon.util.inventory.operation.swapWith
import studio.coni.epsilon.util.items.blockBlacklist
import studio.coni.epsilon.util.math.RotationUtils.getRotationTo
import studio.coni.epsilon.util.math.RotationUtils.yaw
...
import studio.coni.epsilon.util.pause.OffhandPause
import studio.coni.epsilon.util.pause.withPause
import studio.coni.epsilon.util.spoofUnSneak
...
import studio.coni.epsilon.util.world.getHitVecOffset
import studio.coni.epsilon.util.world.getMiningSide
import studio.coni.epsilon.util.math.VectorUtils
import studio.coni.epsilon.event.events.*
import studio.coni.epsilon.util.inventory.slot.*
...
//            renderer.render(false)
    ...
    private inline fun SafeClientEvent.update() {
        if (player.dimension == 0 || !player.allSlots.hasItem(Items.BED)) {
            ...
            studio.coni.epsilon.util.threads.defaultScope.launch {
                val info = calcDamage()
                val list = ArrayList<ESPRenderer.Info>()

                if (info != null) {
                    if (renderBase) list.add(ESPRenderer.Info(info.boxBase, baseColor))
                    if (renderFoot) list.add(ESPRenderer.Info(info.boxFoot, footColor, studio.coni.epsilon.util.graphics.mask.EnumFacingMask.ALL xor studio.coni.epsilon.util.graphics.mask.EnumFacingMask.getMaskForSide(info.side)))
                    if (renderHead) list.add(ESPRenderer.Info(info.boxHead, headColor, studio.coni.epsilon.util.graphics.mask.EnumFacingMask.ALL xor studio.coni.epsilon.util.graphics.mask.EnumFacingMask.getMaskForSide(info.side.opposite)))
                }

    ...
    private fun AsyncRenderer.renderModuleList(
        ...
        if (GUIManager.isRainbow) {
            if (rainbowTimer.passed(10)) {
                hue += GuiSetting.rainbowSpeed.value / 1000f
                rainbowTimer.reset()
            }
            if (hue > 1.0F) {
                hue = 0.0F
        ...
        for ((index, info) in display.withIndex()) {
            ...
            drawStringWithShadow(info.string, (posX - stringXOffset).toFloat(), posY.toFloat() + (height.toFloat() / 2f) - (MainFontRenderer.getHeight() / 2f), color, 1.06f)

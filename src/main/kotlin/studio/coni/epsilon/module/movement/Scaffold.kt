...
import studio.coni.epsilon.common.extensions.isPlaceable
...
import studio.coni.epsilon.util.RotationUtil.legitYaw
import studio.coni.epsilon.util.RotationUtil.yaw
import studio.coni.epsilon.util.extension.AxisAlignedBB.interp
import studio.coni.epsilon.util.extension.block
    ...
    init {
        ...
        safeListener<OnUpdateWalkingPlayerEvent.Pre> {
            runSafe {
                ...
                if (blockData != null) {
                    targetRotation = if (mode == Mode.Hypixel) {
                        Vec2f(player.legitYaw(blockData.face.opposite.yaw), 82.545698881269785867f)
                    } else {
                        val rotationTo = RotationUtil.getBlockRotations(blockData.position, blockData.face)
                        ...
                        val smooth = if (mode == Mode.Hypixel) RotationUtil.faceEntitySmooth(
                        ...
                        if (tower) {
                            if (blockInHotbar != -1 && mc.gameSettings.keyBindJump.isKeyDown && !(mc.player.moveForward != 0f && mc.player.moveStrafing != 0f)) {
                ...
                lastRotation?.let {
                    if (blockData != null || ticks <= rotationTimeout) {
                        sendPlayerPacket {
                            rotate(it)
                        }
                    }
                ...
                blockBelowData = blockData
                playerUnderPos = underPos.also { placingPos = it }
            }
        }
        ...
        safeListener<OnUpdateWalkingPlayerEvent.Post> {
            runSafe {
                ...
                val underPos = playerUnderPos ?: return@runSafe
                lastRotation ?: return@runSafe
                //cast the rotation is correct
                if (if (mode == Mode.Hypixel) (PlayerPacketManager.prevRotation.y == 82.545698881269785867f && PlayerPacketManager.rotation.y == 82.545698881269785867f)
                    else mc.world.getBlockState(underPos).material.isReplaceable
                ) {
                    blockBelowData?.let { blockData ->
                        if (ticks < delay) return@let
                        if (!desyncPlacement && lastPos == blockData.position && ticks < placeTimeout) return@let
                        ...
                        if (blockInHotbar != -1) {
                            ...
                            mc.player.inventory.currentItem = blockInHotbar
